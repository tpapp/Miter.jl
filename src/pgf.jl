...
`bounding_box` is a rectangle that determines the bounding box, with the given `baseline`.

After the preamble, bounding box calculations are suspended.
"""
function preamble(io::IO, bounding_box::Rectangle;
                  standalone::Bool, baseline::LENGTH = LENGTH(0))
...
""")
    ...
    print(io, "\\pgfusepath{use as bounding box}\n\\pgfsetbaseline{")
    _print(io, baseline)
